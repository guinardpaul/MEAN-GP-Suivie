...
export class DashboardComponent implements OnInit {
  ...
  getAllFactureGlobal() {
    ...
    this.factureGlobalService.getAllFactureGlobal().subscribe(
      data => {
        console.log(data);
  ...
  getAllFactureGlobalbyClient(id: number) {
    this.listFactureGlobal = [];
    this.factureGlobalService.getAllFactureGlobalByClient(id).subscribe(
      data => {
        this.listFactureGlobal = data;
        // Set charts data
        for (const f in data) {
  ...
  getAllValidFactureGlobalbyClient(id: number) {
    ...
    this.factureGlobalService.getAllFactureGlobalByClient(id).subscribe(
      data => {
        ...
        if (this.listFactureGlobal.length > 0) {
          this.getAllValidFactureAccomptebyFactureGlobal(
            this.listFactureGlobal[0]._id
          );
        } else {
          this.listFactureAccompte = [];
        }
  ...
  getAllValidFactureAccompte() {
    this.listFactureAccompte = [];
    this.factureAccompteService.getAllFactureAccompte().subscribe(
      data => {
        for (const f in data) {
          if (data.hasOwnProperty(f)) {
            if (data[f].valid) {
  ...
  loadData() {
    ...
    setTimeout(() => {
      ...
      this.pieChartData = [
        ...
        this.montantFactureTTC,
        this.montantRegleTTC
      ];
  ...
  public lineChartData: Array<any> = [
    {
      data: [65, 59, 80, 81, 56, 55, 40, 51, 21, 51, 61],
      label: 'Montant Total TTC'
    },
    {
      data: [28, 48, 40, 19, 86, 27, 90, 51, 21, 51, 61],
      label: 'Montant Facturé TTC'
    },
    {
      data: [18, 48, 77, 9, 100, 27, 40, 51, 21, 51, 61],
