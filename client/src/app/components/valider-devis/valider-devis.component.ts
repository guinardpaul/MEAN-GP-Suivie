...
export class ValiderDevisComponent implements OnInit {
  ...
  getOneDevis(id: number) {
    this.devisService.getOneDevis(id).subscribe(
      devis => {
        this.devis = devis;
  ...
  validerDevis() {
    ...
    newFacture.artisan = this.id_artisan;
    ...
    this.factureGlobalService.getAllFactureGlobalByClient(client._id).subscribe(
      FactureGlobals => {
        ...
        if (FactureGlobals !== null) {
          ...
          if (
            (status_facture && this.client.status_client !== true) ||
            (!status_facture && this.client.status_client === true)
          ) {
            this.clientService
              .updateStatus(client, status_facture)
              .subscribe(
  ...
  verifRef() {
    this.factureGlobalService
      .getOneFactureGlobalByRef(
        this.client._id,
        this.validerDevisForm.get('ref_factureGlobal').value
      )
      .subscribe(
        data => {
          if (data.success) {
  ...
  calculMontant() {
    if (
      !(this.validerDevisForm.controls['montantHt'].value === '') &&
      !(this.validerDevisForm.controls['tauxTva'].value === '')
    ) {
      const montantTTC =
        this.validerDevisForm.controls['montantHt'].value *
        (1 + this.validerDevisForm.controls['tauxTva'].value / 100);
      this.validerDevisForm.controls['montantTtc'].setValue(
